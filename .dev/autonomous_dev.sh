...
AUTONOMOUS_GOAL="${AUTONOMOUS_GOAL:-Довести BTCUSDT breakout research profile conservative-v1-m15-slope-positive-max-trades-8 с текущих 5/8 до 8/8 passed quarterly windows за 2023q1..2024q4 по configured research thresholds, без ослабления порогов, без private/live API и без production approval. Приоритетный следующий поиск: path-risk / exit / stop / holding гипотезы, потому что entry filters, volatility regime filters и confirmation filters уже проверены и не улучшили 5/8 reference.}"
...
Эта цель является явно сформулированной локальной задачей. Не останавливайся с отчетом "локальных задач нет", пока не проверишь, есть ли в `openspec/specs/*`, `openspec/changes/archive/*`, docs, tests или текущем коде нерешенная BTCUSDT quarterly-гипотеза, незакрытая recommendation, deferred research gate, failing quarterly scorecard или очевидный следующий узкий OpenSpec change, который прямо служит этой цели. Отсутствие active OpenSpec change само по себе не означает отсутствие задачи: если цель требует следующего шага, создай ровно один узкий OpenSpec change для проверки/реализации текущей гипотезы и работай через него.
...
Quarterly scorecard protocol обязателен для BTCUSDT breakout research:
- `8/8` означает восемь квартальных окон: `2023q1`, `2023q2`, `2023q3`, `2023q4`, `2024q1`, `2024q2`, `2024q3`, `2024q4`;
- до правок зафиксируй quarterly scorecard: статус каждого квартала (`pass/fail/unknown/blocked`), failed-window blockers, ключевые метрики, artifact path и команду проверки;
- текущий reference из архива: `conservative-v1-m15-slope-positive-max-trades-8`, `5/8 passed`, failing quarters `2024q1`, `2024q2`, `2024q4`, `hypothesis_supported=false`;
- перед каждой итерацией выбери один главный failing quarter или общий механизм провала, который сильнее всего мешает `8/8`;
- после каждой существенной правки обнови quarterly scorecard и явно покажи, какие кварталы изменили статус и приблизился ли счет к `8/8`;
- не архивируй, не коммить и не выходи как success, пока quarterly scorecard не стала `8/8` или пока не зафиксирован честный external blocker.
...
Если BTCUSDT quarterly scorecard достигла `8/8`, все восемь кварталов прошли configured research thresholds, финальная OpenSpec validation выполнена, archive/commit завершены или честно объяснено почему commit невозможен, отправь Telegram success notification командой:
hermes send --to telegram --subject "[StockSeeker autonomous success: 8/8 quarters]" "<короткий отчет>"

В success-сообщении укажи: change-id, профиль/гипотезу, список прошедших кварталов `2023q1..2024q4`, ключевые research thresholds/метрики, artifact paths или summary path, commit hash если создан, и подтверждение что push/MR/merge/live trading/private API не выполнялись. Не отправляй секреты, токены, приватные значения или большие логи. Если Telegram-отправка не сработала, запиши ошибку отправки в финальный отчет и в лог, но не скрывай факт успешного `8/8`.
...
2. Если уже есть начатая локальная фича/OpenSpec change, сначала продолжай ее. Иначе выбери только из уже сформулированных ранее задач или из текущей автономной цели владельца. Если active change нет, но цель указывает на нерешенную гипотезу, создай ровно один узкий OpenSpec change с явной scorecard `8/8`, затем продолжай этот change. Если после bounded search таких задач нет, остановись с отчетом "локальных задач нет".
...
11. Если завершенная фича довела BTCUSDT quarterly scorecard до `8/8`, отправь Telegram success notification после финальной validation/archive/commit стадии и до финального локального отчета. Не отправляй success notification для частичного улучшения вроде `6/8` или `7/8`, для diagnostic-only результата, или для "локальных задач нет".
...
Следующий плановый запуск должен продолжить только незавершенную локальную работу, текущую автономную цель владельца или другую уже сформулированную ранее задачу. Если после bounded search нет ни active work, ни проверяемой гипотезы, ни следующего узкого шага к `8/8`, он должен остановиться с отчетом "локальных задач нет".
