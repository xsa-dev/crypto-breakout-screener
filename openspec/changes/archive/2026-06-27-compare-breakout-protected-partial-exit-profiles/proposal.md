...
Archived entry filters, volatility/regime filters, confirmation filters, fixed holds, paired ATR stop/target exits, close-confirmed stops, break-even/trailing whole-position exits, target-only exits, and unprotected partial exits did not reach realistic-cost `8/8`. The remaining bounded path-risk mechanism is protected residual partial exits: realize a fixed favorable partial leg, then protect only the residual runner after the trade has already reached a favorable ATR threshold. This is distinct from unprotected partial exits because the runner no longer remains fully exposed after partial profit capture, and distinct from whole-position break-even/trailing exits because the first favorable leg is banked before residual protection can trigger.

## What Changes
Implement and evaluate exactly these named research-only protected partial-exit profiles for the existing reference:
...
- preserve entry selection, lifecycle gates, M15 slope feature filter, max-trades risk control, regime filters, confirmation filters, and research thresholds unless one of these explicit named protected partial-exit profiles is selected;
- use entry-time ATR and post-entry M15 bars only after a trade exists;
- apply residual break-even/trailing protection only to the still-open fallback runner after at least one configured partial target has filled;
- compute a single aggregate `BacktestTrade` whose gross PnL/costs are the quantity-weighted sum of deterministic partial/protected-residual legs, preserving existing report/equity APIs;
- serialize protected partial settings, leg counts, residual protection reason, and exit reasons in JSON/CSV artifacts separately from gates, filters, risk controls, and verdicts;
...
Baseline score: `5/8`; failing quarters: `2024q1`, `2024q2`, `2024q4`. Primary mechanism for this change: protect residual runner exposure after partial profit capture to reduce failed-window drawdown without adding new entries, new filters, or whole-position adverse stop churn.

## Success Criteria
- The new protected partial-exit settings are disabled by default and do not change default/reference behavior.
