...
Archived path-risk/exit work has falsified fixed holds, paired ATR stops/targets up to `0.75/2.5`, close-confirmed stops without large favorable targets, break-even/trailing exits, small and large target-only exits, unprotected/protected partial exits, and realized drawdown guards under realistic costs. The strongest remaining blocker is `2024q1` max drawdown: large-target profiles made `2024q1` strongly net/PF positive after realistic costs but still failed drawdown (`-0.6480..-0.7416` without guard; `-0.4597..-0.4788` with a realized drawdown guard).

This change tests a distinct path-risk/exit hypothesis: combine larger favorable target capture with a close-confirmed catastrophic stop so losses are clipped only after a bar closes materially below entry, avoiding the churn of tight intrabar ATR stops while preserving the average-trade improvement from large targets.

## What Changes
Implement and evaluate exactly these fixed research-only profiles:

- `conservative-v1-m15-slope-positive-max-trades-8-target-3p0-close-stop-1p0-hold-32`
...
- evaluate the close-confirmed stop on bar closes before favorable target checks for each post-entry bar;
- record exit settings/reasons in existing JSON/CSV artifacts separately from gate, feature-filter, risk-control, regime-filter, confirmation-filter, and verdict dimensions;
...
Baseline score: `5/8`; failing quarters: `2024q1`, `2024q2`, `2024q4`. Primary mechanism for this change: `2024q1` drawdown reduction while preserving large-target net/PF; if `2024q1` fails under realistic costs, the candidate cannot reach `8/8`.
...
- Combined summary: `artifacts/large-target-close-stop-profile-comparison-summary/early-falsified/summary.json`
- Scorecard CSV: `artifacts/large-target-close-stop-profile-comparison-summary/early-falsified/scorecard.csv`

| Candidate | Required quarter run | Status | Net profit | Profit factor | Max drawdown | Blockers |
| --- | --- | --- | ---: | ---: | ---: | --- |
| `target-3p0-close-stop-1p0-hold-32` | 2024q1 | fail | 192930.95 | 1.5554 | -0.7807 | max_drawdown_below_threshold |
| `target-4p0-close-stop-1p0-hold-32` | 2024q1 | fail | 152286.99 | 1.4159 | -0.8824 | max_drawdown_below_threshold |
| `close-target-2p0-close-stop-1p0-hold-32` | 2024q1 | fail | 136480.57 | 1.3881 | -1.5708 | max_drawdown_below_threshold |

Final verdict: falsified/negative evidence. Score did not reach realistic-cost `8/8`; all candidates are early-falsified by required window `2024q1`, and remaining windows are recorded as blocked/not run after early falsification. Compared with the previous large-target/drawdown-guard evidence, the close-stop direction moved `2024q1` farther from the `max_drawdown >= -0.35` threshold, so this direction is stopped.
