...
The BTCUSDT batch research runner SHALL support disabled-by-default, fixed, named exit-profile comparisons for path-risk, stop, break-even, trailing, holding, target-only, partial-exit, and close-confirmed exit hypotheses while preserving the existing reference behavior when no exit profile is selected.

#### Scenario: Partial-exit profile is selected
- **WHEN** a supported partial-exit profile is selected
- **THEN** already-accepted long trades evaluate configured partial favorable intrabar or close-confirmed target thresholds from entry price using entry-time ATR and post-entry M15 bars only
...
- **AND** any unfilled partial quantity and residual runner quantity are closed at the configured maximum-hold fallback close
...
#### Scenario: Partial-exit profile is evaluated against realistic costs
- **WHEN** a partial-exit profile is evaluated for the BTCUSDT quarterly `2023q1..2024q4` scorecard
